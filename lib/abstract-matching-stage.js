...
class AbstractMatchingStage {
	...
	buildMatched({
		...
		hungarian.forEach(([rIndex, cIndex]) => {
			...
			if (gate[trackId] && gate[trackId][detectionId] === true) {
				...
				blocked.push(m);
				this.logger.debug(`Gate[${trackId}][${detectionId}] is blocking matching (value: ${JSON.stringify(details[trackId][detectionId].values)})`);
			}
		});
	...
	*/
	...
	/**
		...
		*/
	static translateMatchIds({localMatched, localTrackIds, localDetectionIds}) {
		const matched = [];
