...
module.exports = function ({
	...
	while (keepOnGoing && tracks.length > 1) {
		...
		if (allDistancesWithIndexes.length === 0) {
			...
			// Console.log(`${mergeId} Merge Tracks ${rowIndex}[${getTrackStats({track: tracks[rowIndex]}).count}] <-> ${colIndex}[${getTrackStats({track: tracks[colIndex]}).count}], with distance = ${distance}, threshold = ${threshold}`, distances);
